  ...
  <div style="max-width:600px;margin:0 auto;background:#fff;border-radius:8px;overflow:hidden;border:1px solid #e0e0e0">
    ...
    <div style="padding:28px">
      <table style="width:100%;border-collapse:collapse">
        ...
        ${row("Phone", `<a href="tel:${submission.phone}" style="color:#F5C400">${submission.phone}</a>`)}
        ${row("Email", `<a href="mailto:${submission.email}" style="color:#F5C400">${submission.email}</a>`)}
        ${row("Location", submission.location)}
        ${row("Crew Type", submission.crew_type)}
        ${row("Event Dates", submission.event_dates)}
        ${submission.message ? row("Message", submission.message) : ""}
      </table>
    </div>
    <div style="background:#f9f9f9;padding:20px 28px;border-top:1px solid #e0e0e0">
      <a href="tel:${submission.phone}" style="display:inline-block;background:#F5C400;color:#111;padding:10px 20px;border-radius:5px;text-decoration:none;font-weight:700;font-size:13px;margin-right:8px">📞 Call Now</a>
      <a href="mailto:${submission.email}?subject=Re: Crew Availability Enquiry" style="display:inline-block;background:#111;color:#fff;padding:10px 20px;border-radius:5px;text-decoration:none;font-weight:700;font-size:13px;margin-right:8px">✉️ Reply</a>
